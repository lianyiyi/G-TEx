from utils.preprocessor_for_faith import Esnli_preprocessor, Ecqa_preprocessor, Cose_preprocessor, label_dict, Comve_preprocessor
...
class ExpDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        word = self.data_list[index].word
        ...
        return Data(num_nodes=len(input_ids), input_ids=input_ids, attention_mask=attention_mask, edge_index=edge_index,
                    target_input_ids=target_ids, target_attention_mask=target_attention_mask, input_text=text, target_text=target, label=label, word=word)
